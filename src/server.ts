...
const locations = new Queue('schedules fetching and updating', redisConfig);
logger.log('info', 'Created locations schedules queue.');
locations
  .on('error', error => {
    logger.log('error', `Schedule job failed with ${error.toString()}`);
  })
  .on('stalled', jobber => {
    logger.log('warn', `Job ${jobber.id} schedule queue stalled.`);
  })
  .on('failed', (jobber, error) => {
    logger.log('error', `Job ${jobber.id} schedule queue failed with ${error.toString()}`);
  })
  .empty()
  .then(() => {
    logger.log('info', 'Cleared locations schedules queue.');
    locations.process(locationSchedulesPath);
    locations.add({ days: 10 }, { repeat: { cron: job.locationsScheduleCron } });
